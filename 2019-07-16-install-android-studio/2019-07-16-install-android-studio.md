...
Теперь нам предложат выбрать устройство, на котором будем запускать приложение. Пока у нас в списке два эмулятора, причем один находится наверху (в разделе `Connected Devices`), так как он у нас включен (вы же его не выключили?):

![Запуск приложения](img/run_03.png)

Если вы выключили эмулятор, то можно либо его запустить через AVD Manager, либо тут выбрать эмулятор и по нему дважды щелкнуть. Но если у вас устройство медленное, и эмулятор запускается долго, то лучше его запустить заранее.
...
Но оно не получится запустить приложение на нем, так как устройство имеет подпись `UNAUTHORIZED` (или `Offline`). Это возникает по причине того, что на самом устройстве мы не разрешили компьютеру подключение. Разрешите:
